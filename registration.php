...
if (isset($_POST["register"])) {
    ...
    $sqlUsername = "SELECT * FROM users WHERE full_name = ?";
    
    $stmtEmail = mysqli_stmt_init($conn);
    $stmtUsername = mysqli_stmt_init($conn);

    if (mysqli_stmt_prepare($stmtEmail, $sqlEmail) && mysqli_stmt_prepare($stmtUsername, $sqlUsername)) {
        mysqli_stmt_bind_param($stmtEmail, "s", $email);
        mysqli_stmt_execute($stmtEmail);
        $resultEmail = mysqli_stmt_get_result($stmtEmail);

        mysqli_stmt_bind_param($stmtUsername, "s", $fullName);
    ...
    if (count($errors) > 0) {
        ...
        if (mysqli_stmt_prepare($stmt, $sql)) {
            ...
            echo "<div class='alert alert-success'>You are registered successfully. Now you can log in.</div>";
        } else {
            die("Database error: " . mysqli_error($conn));
        }
    }
            ...
            <div class="row g-5">
                <div class="col-lg-7">
                    <form action="registration.php" method="post">
                        <div class="row g-3">
                            <div class="col-12">
                                <input type="text" class="form-control bg-light border-0 px-4" placeholder="Full Name" name="fullname" style="height: 55px;">
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                <input type="password" class="form-control bg-light border-0 px-4" placeholder="Repeat Password" name="repeat_password" style="height: 55px;">
                            </div>
                            <div class="col-12">
